...
def init_db():
    ...
    if first_run:
        messagebox.showinfo("Welcome", "Welcome to Vocabs! Please first add a few words (at least one) to your personal dictionary.") 
        add_btn()
    ...
    def display_words(): # toplevel win to display data
        def see(event): # button for seeing the details for the word selected
            ...
            if type_ == "verb" or type_ == "v":
                ...
                details = "{} {} ({}, {}): {}".format(article, word, gender, type_, meaning)
            elif type_ != "":
                details = "{} ({}): {}".format(word, type_, meaning)
            else:
